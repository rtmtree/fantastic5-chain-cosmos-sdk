...
func TestSystemInfoQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			response: &types.QueryGetSystemInfoResponse{SystemInfo: msgs[0]},
		},
		{
			desc: "Second",
			request: &types.QueryGetSystemInfoRequest{
			...
			response: &types.QueryGetSystemInfoResponse{SystemInfo: msgs[1]},
		},
		{
			desc: "KeyNotFound",
			request: &types.QueryGetSystemInfoRequest{
...
func TestSystemInfoQueryPaginated(t *testing.T) {
	...
	request := func(next []byte, offset, limit uint64, total bool) *types.QueryAllSystemInfoRequest {
		return &types.QueryAllSystemInfoRequest{
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.SystemInfoAll(wctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.SystemInfoAll(wctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.SystemInfo), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := keeper.SystemInfoAll(wctx, request(nil, 0, 0, true))
		require.NoError(t, err)
	...
	t.Run("InvalidRequest", func(t *testing.T) {
		_, err := keeper.SystemInfoAll(wctx, nil)
