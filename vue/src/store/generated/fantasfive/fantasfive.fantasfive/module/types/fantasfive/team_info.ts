...
const baseTeamInfo: object = { nextId: 0 };

export const TeamInfo = {
  encode(message: TeamInfo, writer: Writer = Writer.create()): Writer {
    if (message.nextId !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): TeamInfo {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseTeamInfo } as TeamInfo;
  ...
  fromPartial(object: DeepPartial<TeamInfo>): TeamInfo {
    const message = { ...baseTeamInfo } as TeamInfo;
    if (object.nextId !== undefined && object.nextId !== null) {
